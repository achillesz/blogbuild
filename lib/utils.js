...
function renderPost(dir, file) {
    ...
    post.date = moment(new Date(post.date)).format('LLL')
    post.layout = post.layout || 'post';
    var config = loadConfig(dir);
    var layout = path.resolve(dir, '_layout', post.layout + '.html'); // 找到对应的模版文件路径
...
function renderIndex(dir) {
    ...
    eachSourceFile(sourceDir, function(f, s) {
        ...
        post.date = moment(new Date(post.date)).format('YYYY-MM-DD');
        post.url = '/posts/' + stripExtname(f.slice(sourceDir.length + 1)) + '.html'; // 遍历md文件， 去掉扩张名，换成html, 用作跳转链接
        list.push(post);
    });
